...
//
//  修复：补齐了所有被精简的遗留属性 getter/setter，完美通过 -Wincomplete-implementation 检查。
//
...
static NSAttributedString *formattedFPSAttributedString(BOOL isFocused) {
    @autoreleasepool {
        ...
        if (needsFPSBaselineReset) {
            ...
            return [[NSAttributedString alloc] initWithString:@"0 FPS" attributes:@{
...
- (NSDictionary *)extraUserDefaultsDictionary {
    ...
    dispatch_once(&onceToken, ^{
      isJailbroken = [[NSFileManager defaultManager] fileExistsAtPath:JBROOT_PATH_NSSTRING(@"/Library/PreferenceBundles/TrollSpeedPrefs.bundle")];
    });
    if (!isJailbroken) { return nil; }
...
- (void)updateViewConstraints {
    ...
    const CACornerMask kCornerMaskBottom = kCALayerMinXMaxYCorner | kCALayerMaxXMaxYCorner;
    const CACornerMask kCornerMaskAll = kCALayerMinXMinYCorner | kCALayerMaxXMinYCorner | kCALayerMinXMaxYCorner | kCALayerMaxXMaxYCorner;
    ...
    if (isCenteredMost && !isPad && !isLandscape) {
        ...
        CGFloat minimumTopConstraintConstant = isLandscape ? (isPad ? 30 : 10) : 20; 
        if (!isLandscape && !isPad) {
            if (CGRectGetMinY(layoutGuide.layoutFrame) >= 51) minimumTopConstraintConstant = -8;
            else if (CGRectGetMinY(layoutGuide.layoutFrame) > 30) minimumTopConstraintConstant = -12;
        }
        
        _topConstraint = [_contentView.topAnchor constraintEqualToAnchor:(isLandscape ? self.view.topAnchor : layoutGuide.topAnchor) constant:minimumTopConstraintConstant + realCustomOffsetY];
...
- (void)panGestureRecognized:(UIPanGestureRecognizer *)sender {
    ...
    if (sender.state == UIGestureRecognizerStatePossible || sender.state == UIGestureRecognizerStateBegan) {
        ...
        if (sender.state == UIGestureRecognizerStateEnded) {
            CGPoint trans = [sender translationInView:sender.view.superview];
            CGFloat newX = beginX + trans.x;
            CGFloat newYToSave = _topConstraint ? (beginY + trans.y - 20) : (beginY + trans.y);
            ...
            [_userDefaults setObject:@(newYToSave) forKey:HUDUserDefaultsKeyRealCustomOffsetY];
            [self saveUserDefaults];
        }
